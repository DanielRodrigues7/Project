$(document).ready(function(){
	...
	var nome = $("#Nome_Nut").val();
	var email = $("#Email_Nut").val();
	var especialidade = $("#Especialidade_Nut").val();
	var nasc = $("#nascimento_Nut").val();
	var tel = $("#telefone_nut").val();

	
	$("#btnCadastrarNut").click(function(){
		var nome1 = $("#Nome_Nut").val();
		var email1 = $("#Email_Nut").val();
		var especialidade1 = $("#Especialidade_Nut").val();
		var nasc1 = $("#nascimento_Nut").val();
		var tel1 = $("#telefone_nut").val();

		if($.trim(nome) == $.trim(nome1) &&
			$.trim(email) == $.trim(email1) &&
			$.trim(especialidade) == $.trim(especialidade1) &&
			$.trim(nasc) == $.trim(nasc1) &&
			$.trim(tel) == $.trim(tel1)){
			swal("Não houve nenhuma alteração!");
			return false;
